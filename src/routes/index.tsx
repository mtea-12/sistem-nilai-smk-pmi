...
function HalamanMasuk() {
  ...
  function kirim(e: React.FormEvent) {
    e.preventDefault();
    const hasil = masuk(username, sandi);
    if (!hasil) {
      toast.error("Nama pengguna atau kata sandi salah.");
      return;
    }
    toast.success(`Selamat datang, ${hasil.nama}`);
    navigate({ to: "/dasbor" });
  }
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <section className="relative hidden flex-col justify-between bg-sidebar p-10 text-sidebar-foreground lg:flex">
        ...
        <p className="text-xs opacity-70">{SEKOLAH.alamat}</p>
      </section>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
            ...
            <form onSubmit={kirim} className="mt-6 space-y-4">
              ...
              <button
                type="submit"
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary-dark"
              >
          ...
          <div className="mt-6 rounded-2xl border border-dashed border-border bg-card/60 p-5">
            <p className="text-sm font-semibold">Akun demo — klik untuk mengisi otomatis</p>
            <div className="mt-3 grid gap-2">
              {akunDemo.map((a) => (
                <button
                  ...
                  className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-border bg-background px-3 py-2.5 text-left transition-colors hover:border-primary"
                >
                  <span className="min-w-0">
                    <span className="block truncate text-sm font-medium">{labelPeran[a.peran]}</span>
